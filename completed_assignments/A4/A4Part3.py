...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    xMX= stft.stftAnal(x, w, N, H)[0]
    frameN = xMX.shape[0]
    binFreq = fs/N
    k1 = int(3000//binFreq)
    k2 = int(10000//binFreq)
    # print('binFreq = ' , binFreq)
    # print('len(x) = ', len(x))
    # print('fs =' , fs)
    # print('frameN = ', frameN)
    # print('xMX Shape: ', xMX.shape)
    # print('k1 = ', k1)
    # print('k2 =' , k2)
    # print('lenx/hop = ',  int(len(x)/H))
    
    # Compute energy for each slice (put slice[:k , : ]) in first column of 
    # output. and the rest in the second column
    
    xMX = 10**(xMX/20)
    Elo = np.zeros(frameN)
    Ehi = np.zeros(frameN)
    Elo[:] = np.sum(abs(xMX[: , 1:k1+1])**2, axis = 1)
    Ehi[:] = np.sum(abs(xMX[: , k1+1:k2+1])**2, axis = 1)
    Elo[Elo < eps] = eps
    Ehi[Ehi < eps] = eps
    Elo = 10 * np.log10(Elo)
    Ehi = 10 * np.log10(Ehi)
    ...
    engEnv = np.zeros(shape = (frameN, 2))
    engEnv[:, 0] = Elo
    engEnv[: , 1] = Ehi
