...
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming', 
    ...
    hfreq = HM.harmonicModelAnal(x, fs, w, N, H, t, nH, minf0, maxf0, f0et, harmDevSlope=0.01, minSineDur=0.0)[0]
    # f0 = HM.f0Detection(x, fs, w, N, H, t, minf0, maxf0, f0et)
    # 2. Extract the time segment in which you need to compute the inharmonicity. 
    start = int(np.ceil((t1 * (fs/H))))
    end = int(np.floor(t2 * (fs/H)))
    hfreq_seg = hfreq[start : end + 1]
    f0_seg = hfreq_seg[:, 0]
    
    #print(type(hfreq_seg))
    #print('--------\n', 'f0:\n\n', f0_seg)
    ...
    I = []
    for i in range(hfreq_seg.shape[0]):
    #     print('--------\n', 'harmonic differences: ', (hfreq_seg[i] - r * f0_seg[i])/r)
        # hfreq_all = hfreq_seg[i , :]
        hfreq_nonzero = hfreq_seg[i , :][hfreq_seg[i , :] != 0]
        # print('--------\n', 'hfreq_nonzero: \n', len(hfreq_nonzero))
        # print('--------\n', 'hfreq_all: \n', len(hfreq_all))
        R = len(hfreq_nonzero)
        r = np.linspace(1, R, R, dtype = int)
        #print('r: ', r)
        #print('R =: ', R)
        I.append((1/R) * np.sum((abs(hfreq_nonzero - f0_seg[i]*r))/r))
        #print('---------------------------------')
        

    #print('--------\n', 'I:\n\n', I)
    
    I_mean = np.sum(I)/(end + 1 - start + 1)
