...
def minFreqEstErr(inputFile, f):
    ...
    def iterFreqEst (x, fs, M, N, window):
    
        # select a frame of size M from inputFile centered at 0.5s
        midIndex = int(0.5/(1/fs)-1)
        frame = x[midIndex - int(M//2) : midIndex + int((M+1)/2)]
        assert len(frame) == M
        
        #Compute DFT of the slice:
        w = get_window(window, M)
        mX, pX = DFT.dftAnal(frame, w, N)
        
        # Calculate fp
        ploc = UF.peakDetection(mX, t)
        fp= UF.peakInterp(mX, pX, ploc)[0] * (fs/N)
        err_amt = np.abs(fp - f)
        good_enough = (err_amt < 0.05)
        return good_enough, fp
    ...
    #set initial M and N
    M = int((fs/f)//100) * 100 + 1
    N = int(2**np.ceil(np.log2(M)))
    
    while iterFreqEst (x, fs, M, N, window)[0] == False:
        M += 100
        N = int(2**np.ceil(np.log2(M)))
        
    fp = iterFreqEst (x, fs, M, N, window)[1][0]
